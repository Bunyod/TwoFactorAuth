...
object Base32 {
  ...
  /**
    ...
    */
  def collapseBinary(bits: List[Int]): Int =
    bits.foldRight((0,1)){
      case (b,(acc,value)) => (acc|(b*value),value << 1)
    } match { case (res, _) => res }
  ...
  def encode(s: String): Array[Byte] = {
    ...
    (List.fill(5 - ((bytes.size*8) % 5))(0) ++ // left padding with zero bits
      expandBinary(bytes))
      .grouped(5)
